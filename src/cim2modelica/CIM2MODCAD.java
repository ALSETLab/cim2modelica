...
public class CIM2MODCAD {
	...
	if (args[0].equals("-d")) {
	    ...
	    cartografo = new ModelCADDesigner(
		    ...
		    profiFact.getProfile(lector.get_source_DY_profile(), CIMProfileType.DIAGRAM_LAYOUT));
	    constructor = new ModelCADBuilder(args[1]);
	    lector = null;
	    profiFact = null;
	}
	if (args[0].equals("-p")) { // read the cim profile files separately
	    cartografo = new ModelCADDesigner(new EQProfileModel(args[2]), new TPProfileModel(args[3]),
		    new SVProfileModel(args[4]), new DYProfileModel(args[5]), new DLProfileModel(args[5]));
	    constructor = new ModelCADBuilder(args[1]);
	}
    }
	...
	Map<Resource, RDFNode> profile_DL = cartografo.load_DL_profile();
	for (Resource key : profile_EQ.keySet()) {
		...
		PwPinMap mapTerminal = cartografo.create_TerminalModelicaMap(key,
			"./res/map/openipsl/connectors/cim_iteslalibrary_pwpin.xml", cimClassResource);
		...
		if (equipmentResource[1].equals("SynchronousMachine")) {
		    ...
		    // GENCLSMap mapSyncMach=
		    // cartografo.create_GENCLSModelicaMap(
		    // cartografo.get_CurrentConnectionMap().getConductingEquipment(),
		    // "./res/map/cim_iteslalibrary_gencls.xml",
		    // equipmentResource);
		    // momachine=
		    // constructor.create_MachineComponent(mapSyncMach);
		    // }
		...
		if (equipmentResource[1].equals("EnergyConsumer") || equipmentResource[1].equals("NonConformLoad")
		    ...
		    MOClass moload = constructor.create_LoadComponent(mapEnergyC, profile_DL);
		...
		if (equipmentResource[1].equals("ACLineSegment")) {
			...
			moline = constructor.create_LineComponent(mapACLine, profile_DL);
			moline.add_Terminal(mopin);
		...
		if (topologyResource[1].equals("TopologicalNode")) {
			...
			mopinbus.set_InstanceName("p"); // trick to set all pin
			mobus.add_Terminal(mopinbus);
			mobus.update_ComponentAnnotation(mapTerminal, profile_DL);
		...
		TransformerEndAuxiliarMap auxiliarTwtMap = cartografo.create_TransformerModelicaMap(key,
			"./res/map/openipsl/branches/cim_iteslalibrary_twowindingtransformer.xml", cimClassResource);
		...
		PwPinMap mapTerminal = cartografo.create_TerminalModelicaMap(auxiliarTwtMap.get_Terminal_Resource(),
			"./res/map/openipsl/connectors/cim_iteslalibrary_pwpin.xml", terminalResource);
		/*
		...
		if (moTransformer != null) {
		    ...
		    moTransformer = constructor.create_TransformerComponent(twtMap, profile_DL);
		    moTransformer.add_Terminal(mopin);
	...
	for (MOPlant plant : constructor.get_Network().get_planta()) {
	    ...
	    constructor.save_ModelicaFile(plant.to_ModelicaClass(), plant.get_Name(), plantPackage);
	}
