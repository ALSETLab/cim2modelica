...
public class CIMModel {
	...
	while (statements.hasNext()) {
	    ...
	    // stmt.getPredicate().getLocalName()); //name of the variable
	    // System.out.println("Value -> "+ stmt.getAlt()); //value of the
	    // variable as String
	    // System.out.println("Literal -> "+ stmt.getAlt().isLiteral());
	    // System.out.println("URIResource -> "+
	    // stmt.getAlt().isURIResource());
	    ...
	    //// this.attribute.put(stmt.getPredicate().getLocalName(),
	    // stmt.getAlt().getLocalName());
	    // }
	}
    ...
    /**
     * cim_name="SvVoltage.angle" cim_name="SvVoltage.v"
     * cim_name="SvPowerFlow.p" cim_name="SvPowerFlow.q"
     * cim_name="BaseVoltage.nominalVoltage" cim_name="BasePower.basePower"
     * 
     * @param _subject
     * @return
     */
	...
	StmtIterator svPowerFlowAtts, topologicalNodeAtts, svVoltageAtts, baseVoltage;
	...
	while (terminalAttributes.hasNext()) {
		...
		if (terminalAttribute.getPredicate().getLocalName().equals("Terminal.SvPowerFlow")) {
		    ...
		    svPowerFlowAtts = terminalAttribute.getAlt().listProperties();
		...
		if (terminalAttribute.getPredicate().getLocalName().equals("Terminal.TopologicalNode")) {
		    topologicalNodeAtts = terminalAttribute.getAlt().listProperties();
			...
			if (topoNodeAttribute.getPredicate().getLocalName().equals("TopologicalNode.SvVoltage")) {
			    svVoltageAtts = topoNodeAttribute.getAlt().listProperties();
			    while (svVoltageAtts.hasNext()) {
				svVoltAttribute = svVoltageAtts.next();
				if (svVoltAttribute.getAlt().isLiteral()) {
				    this.attribute.put(svVoltAttribute.getPredicate().getLocalName(),
					    svVoltAttribute.getString());
				}
			    }
			    svVoltageAtts.close();
			    svVoltageAtts = null;
			...
			if (topoNodeAttribute.getPredicate().getLocalName().equals("TopologicalNode.BaseVoltage")) {
			    baseVoltage = topoNodeAttribute.getAlt().listProperties();
		    ...
		    /*
		     * Add the rfd_id of the TopologicalNode which Terminal is
		     * related to
		     */
		    this.attribute.put(terminalAttribute.getPredicate().getLocalName(),
		...
		if (terminalAttribute.getPredicate().getLocalName().equals("Terminal.ConductingEquipment")) {
		    /*
		     * Add the rfd_id of the CondictingEquipment which Terminal
		     * is related to
		     */
		    this.attribute.put(terminalAttribute.getPredicate().getLocalName(),
			    terminalAttribute.getResource());
	...
	while (iAttributes.hasNext()) {
		...
		if (attributeClass.getPredicate().getLocalName()
			.equals("SynchronousMachine.SynchronousMachineDynamics")) {
		    dynamicAtts = attributeClass.getAlt().listProperties();
		    while (dynamicAtts.hasNext()) {
			classAttribute = dynamicAtts.next();
			// System.out.println("In CIM ... "+
			// classAttribute.getAlt().toString());
			if (classAttribute.getAlt().isLiteral()
				&& !classAttribute.getPredicate().getLocalName().equals("IdentifiedObject.name")) {
			    this.attribute.put(classAttribute.getPredicate().getLocalName(),
				    classAttribute.getString());
			}
			if (classAttribute.getAlt().isURIResource() && classAttribute.getPredicate().getLocalName()
				.equals("SynchronousMachineTimeConstantReactance.rotorType")) {
			    this.attribute.put(classAttribute.getPredicate().getLocalName(),
	...
	while (iAttributes.hasNext()) {
	    attributeClass = iAttributes.next();
	    if (attributeClass.getAlt().isLiteral()
		    && !attributeClass.getPredicate().getLocalName().equals("IdentifiedObject.name")) {// Take
												       // the
												       // literals
												       // but
												       // not
		...
		if (attributeClass.getPredicate().getLocalName().equals("PowerTransformerEnd.PowerTransformer")) {
		    // From this class, the interest is in the RFD_ID value,
		    // since there is one transformer with multiple Ends
