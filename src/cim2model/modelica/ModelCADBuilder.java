...
public class ModelCADBuilder 
{
	...
	public MOConnector create_PinConnector(PwPinMap _terminalMap)
	{
		...
		while (imapAttList.hasNext()) {
			...
			if (current.getCimName().equals("ACDCTerminal.sequenceNumber")) {
				if (current.getContent().equals("1"))
						pin.set_InstanceName("p");
				if (current.getContent().equals("2"))
		...
		while(iConnections.hasNext())
		{
			...
			else
			{
				conexio= new MOConnectNode(equipment.get_InstanceName(), 
						...
						bus.get_InstanceName(), 
						bus.get_Terminal(currentConnection.get_T_id()).get_InstanceName());
					conexio.set_CoordEQ(currentConnection.getCe_pos()[0],
							currentConnection.getCe_pos()[1]);
					conexio.set_CoordBUS(currentConnection.getTn_pos()[0],
							currentConnection.getTn_pos()[1]);
			}
